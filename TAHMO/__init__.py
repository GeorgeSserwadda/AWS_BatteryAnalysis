...
class apiWrapper(object):
    ...
    def getMeasurements(self, station, startDate=None, endDate=None, variables=None, dataset='raw'):
                    ...
                    if 'series' in result and len(result['series']) >= 1 and 'values' in result['series'][0]:
                        for serie in result['series']:
                            ...
                            for shortcode in shortcodes:
                                ...
                                timeserie = list(map(lambda x: [x[time_index], x[value_index], x[sensor_index], x[quality_index]],
        ...
        for shortcode in seriesHolder:
            ...
            if len(timestamps) > len(set(timestamps)):
                ...
                for sensor in sensors:
                    ...
                    quality = list(map(lambda x: x[3], sensorSerie))  # Get quality values
                    serie = pd.Series(values, index=pd.DatetimeIndex(timestamps), name='%s_%s' % (shortcode, sensor))

                    # Create DataFrame for this sensor including quality
                    df_sensor = serie.to_frame()
                    df_sensor['quality'] = quality
                    series.append(df_sensor)
                ...
                quality = list(map(lambda x: x[3], seriesHolder[shortcode]))  # Get quality values
                serie = pd.Series(values, index=pd.DatetimeIndex(timestamps), name=shortcode)

                # Create DataFrame including quality
                df = serie.to_frame()
                df['quality'] = quality
                series.append(df)
